...
pub struct Mmu {
    use_bios: bool,
    rom_bios:             [u8; 256],    // $0000 - 0x0100
    rom_cartridge_bank_0: [u8; 0x4000], // $0000 - $3FFF
    rom_cartridge_bank_n: [u8; 0x4000], // $4000 - $7FFF
    ram_character:        [u8; 0x1800], // $8000 - $97FF
    ram_tile_map:         [u8; 0x0800], // $9800 - $9FFF
    ram_cartridge:        [u8; 0x2000], // $A000 - $BFFF
    ram_internal_bank_0:  [u8; 0x1000], // $C000 - $CFFF
    ram_internal_bank_n:  [u8; 0x1000], // $D000 - $DFFF
    ram_object_attribute: [u8; 40 * 4], // $FE00 - $FE9F
}
...
impl Mmu {
    fn new() -> Self {
        Mmu {
            ...
            rom_cartridge_bank_0: [0; 0x4000],
            rom_cartridge_bank_n: [0; 0x4000],
            ram_character:        [0; 0x1800],
            ram_tile_map:         [0; 0x0800],
            ram_cartridge:        [0; 0x2000],
            ram_internal_bank_0:  [0; 0x1000],
            ram_internal_bank_n:  [0; 0x1000],
            ram_object_attribute: [0; 40 * 4],
        }
    }
}

#[test]
fn bios_checksum_tests() {
    let bios: Vec<u8> = Mmu::new().rom_bios.into();
    let digest = format!("{:x}", md5::compute(bios));

    assert_eq!(digest, "32fbbd84168d3482956eb3c5051637f5")
}